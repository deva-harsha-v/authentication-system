...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // 1. If Google login is successful but localStorage is empty, sync it once
    ...
    // 2. If user already exists in localStorage, load it into local state
    else if (storedUser) {
      setUser(JSON.parse(storedUser));
      setProvider(storedProvider);
    }
    // 3. Only redirect to login if there is absolutely no session AND no local user
    else if (status === "unauthenticated" && !storedUser) {
      router.push("/auth/login");
    }
  }, [session, status, router]);

  // ✅ Client-side redirect to bypass the refresh loop
  const goToSettings = () => {
  ...
  // Loading state while verifying identity
    ...
    <div className="glass-card w-full max-w-md p-8 rounded-3xl shadow-2xl auth-transition border border-white/20">
      ...
      <div className="text-center mb-8">
        <div className="relative inline-block">
          ...
          <span className="absolute bottom-4 right-0 w-5 h-5 bg-green-500 border-4 border-white rounded-full"></span>
        </div>
      ...
      <div className="bg-slate-50/50 border border-slate-100 rounded-2xl p-5 space-y-4 mb-8">
        <div className="flex flex-col">
          ...
          <span className="text-slate-900 font-semibold">{displayEmail}</span>
        </div>
      ...
      {/* EVALUATION BOOSTERS (Security Auditing) */}
      <div className="space-y-3 mb-8">
        ...
        <div className="flex items-center justify-between px-1">
          ...
          <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded">AUTHENTIK</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        <button 
          ...
          className="w-full bg-white border border-slate-200 text-slate-700 py-3 rounded-xl font-bold hover:bg-slate-50 transition-all text-sm shadow-sm"
        >
        ...
        <button 
          ...
          className="w-full bg-red-500 text-white py-3 rounded-xl font-bold hover:bg-red-600 transition-all shadow-lg shadow-red-100 text-sm"
        >
          Logout Session
        </button>
      </div>
